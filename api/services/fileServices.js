const create_file = async ({ uploader_id, title }, { pgPool }) => {
  const ret = await pgPool.query('select * from syncollege_db.create_file($1, $2)', [uploader_id, title]);
  return ret.rows[0];
};
...
const update_file = async ({ uploader_id, file_id, title }, { pgPool }) => {
  const ret = await pgPool.query('select * from syncollege_db.update_file($1, $2, $3, $4, $5)', [uploader_id, file_id, title]);
  return ret.rows[0];
};
